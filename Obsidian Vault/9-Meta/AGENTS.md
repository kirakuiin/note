...
### 8.3.1 例外：纯文本替换可绕过 cli

上述强制路由的**唯一例外**：当且仅当所有以下条件**同时**满足时，可以使用通用 `edit` / `write` 工具直接修改 `.md` 文件：

1. **纯字符串替换**——不涉及 wikilink 新增 / 删除 / 改名
2. **不涉及 frontmatter 字段的语义修改**（仅修字面值且无外部 dataview 依赖时可放行）
3. **不涉及文件的移动 / 改名 / 创建 / 删除**——这些操作 Obsidian 必须感知
4. **不修改影响链接图的结构**（headings 的 anchor、block id 等）

典型可绕过场景：
- 修复转义瑕疵（如反引号被 cmd 多层 shell 转义成 `` \` ``、`\n` 误写成字面）
- 修正错别字、措辞润色
- 调整缩进、空行、表格列宽对齐
- 在已有段落内追加/删除几行内容（不含 wikilink）

**仍必须走 cli 的场景**（不在例外范围）：
- 任何包含 `[[...]]` `![[...]]` 的内容增删（哪怕只是改显示文本）
- 文件 CRUD（新建 / 删除 / 移动 / 改名）
- frontmatter 字段的增删改（特别是 `tags` / `aliases` / 自定义 dataview 字段）
- property:set / property:remove 类型的批量元数据操作
- 修改后**必须 `obsidian unresolved total` 比对基线**，确保没引入静默失链

**实战印证（2026-05-06）：**
用 `edit` 工具修复 `_log.md` 中反引号被 cmd 多层 shell 转义成 `` \` `` 的瑕疵，`unresolved` 持平 185。说明此类纯字面修复确实不破坏索引，强制走 cli 是过度保守。
甚至本节自身的引入也踩了同一个坑：用 `obsidian eval` 写本节时反斜杠被吃，改用 `edit` 工具二次修复——**元层印证**了规则的合理性。
