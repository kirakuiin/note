...
- [x] 3.6 **[Batch E]** 迁移 `软件食谱/`、`工具用法/` → `6-Tools/`（按 design D3 保持扁平，文件名加类别前缀如 `编辑器-VSCode.md`）；本批后双重 verify + tag。**完成（task 3.6）**：① `软件食谱/` 实测早已为空（含子目录），无需迁移；② `工具用法/` 13 个文件全部 `obsidian move` 一步完成 move+rename → `6-Tools/<类别>-<工具名>.md` 扁平结构（编辑器-VSCode/编辑器-Emacs/版本控制-Git/版本控制-SVN/笔记-Obsidian/Obsidian插件-Dataview/标记语言-Markdown/排版-LaTeX/测试-NUnit/AI-Cursor/AI-ClaudeCode/学习-Anki/系统-Windows）；③ `6-Tools/_index.md` 按类别分组登记 13 条 wikilink（13 次 `obsidian append` 短串避陷阱 2 + 一次 `edit` 工具清理重复段；落盘 UTF-8 strict 验证通过）；④ `obsidian unresolved` 36 → 36 零失链；⑤ git commit + tag `post-batch-E`。源 `工具用法/` 物理空壳留待 task 3.12 统一删除。
- [x] 3.7 **[Batch F]** 迁移 `开发项目/代号α/` → `3-Projects/个人-代号α/`；本批后双重 verify + tag。**完成（task 3.7）**：① 用户决策去掉「个人-」前缀，目标改为 `3-Projects/代号α/`；② 先 `obsidian create` 占位 `_index.md`（`obsidian move` 不会自动 mkdir 父目录）+ 6 个 `property:set` 填 frontmatter（area=project / visibility=public / status=paused / created/updated=2026-04-29 / tags=[game-design,card-game]）；③ 3 文件 `obsidian move` 一步迁入；④ `edit` 工具补 `_index.md` 正文（按 task 3.6 §1 补丁的判定流程，UTF-8 strict 验证通过）；⑤ `3-Projects/_index.md` 追加 `[[代号α/_index|代号α]]` 入口；⑥ unresolved 36 → 36 零失链。源 `开发项目/` 空壳留 task 3.12 清。
- [x] 3.8 **[Batch G]** 迁移 `Templates/` → `9-Meta/Templates/`；本批后双重 verify + tag。**完成（task 3.8）**：① 实测 `9-Meta/Templates/` 已有 task 2.2 的 7 个通用模板，3 个老模板（英语语法/设计模式/重构手法）文件名无冲突，直接 move；② 同步改 Obsidian 内置 Templates 插件配置 `.obsidian/templates.json`：`folder: "Templates"` → `"9-Meta/Templates"`，并 `obsidian eval` 重载到内存（不重启 Obsidian 即生效）；③ `9-Meta/Templates/_index.md` 拆为「通用模板」+「领域专属模板」两节并补 3 行入口；④ unresolved 36 → 36；源 `Templates/` 空壳留 task 3.12 清。
- [x] 3.9 **[资产目录决策]** ~~`Assets/`、`Drawings/`、`Resources/`、`Excalidraw/` **不动**；在 `9-Meta/AGENTS.md` 显式标注"历史扁平资产目录，新资产走 `2-Wiki/<主题>/_assets/`"~~ **用户拍板改为方案 B 收编（task 3.9 重写完成）**：① 实测 `Drawings/` `Resources/` 完全为空（design 列表过时），合并入 task 3.12 删空目录；② `Assets/` (212 文件 / 46MB) 用 `app.fileManager.renameFile` 整目录原子迁移到 `9-Meta/Assets/`，alwaysUpdateLinks=true 自动更新所有 wikilink/embed 引用（vault 内全 shortest 形式 → 跟随移动）；③ `Excalidraw/` (200 文件 / 7.8MB) 同手法迁到 `9-Meta/Excalidraw/`；④ 同步改 `.obsidian/app.json` 的 `attachmentFolderPath` → `9-Meta/Assets`；改 `.obsidian/plugins/obsidian-excalidraw-plugin/data.json` 的 4 个路径字段（folder / templateFilePath / scriptFolderPath / fontAssetsPath）→ `9-Meta/Excalidraw`；⑤ 实测 5 个 .canvas 内部 `file:` 字段已被 alwaysUpdateLinks 自动改写到新路径（含 2-Wiki/方法论/我的GTD实践笔记.canvas + 2-Wiki/游戏开发/游戏编程模式/📚大纲.canvas）；⑥ 手修 1 处代码块内残留的 `[[Excalidraw/...]]` 引用（alwaysUpdateLinks 不处理代码块内）；⑦ unresolved 36 → 36，check_links 289 → 289 双重 verify 零增量；⑧ `9-Meta/AGENTS.md` §"还允许的根级目录"段改写：删除 `Assets/` `Excalidraw/`，新增"资产存储约定"段（4 个字段配置位置）。
- [x] 3.10 **[Dataview/Canvas 修正]** 按 1.4 清单逐条手工修正：
  - 4 处 Dataview `FROM "..."` 路径
  - `日常杂谈/我的GTD实践笔记.canvas` 内部节点（手工 verify wikilink 是否仍解析正确，必要时在 Obsidian 内打开重保存）

  **完成（task 3.10）**：① 实测 vault 内全部 6 处 Dataview FROM（design 低估 2 处）已在前序 batches 全部修完——A 修 1 处（英语）、B 修 3 处（设计模式 + 重构 ×2）、C-low 修 2 处（Unity 知识点）；② canvas 实测 5 个 .canvas 内部 `file:` 字段在迁移过程中由 alwaysUpdateLinks 自动改写（含 `2-Wiki/方法论/我的GTD实践笔记.canvas` 的 GTD 节点 → `9-Meta/Excalidraw/English grammer/Get Things Done...`，以及 `2-Wiki/游戏开发/游戏编程模式/📚大纲.canvas` 的 `游戏问题解决流程` 节点 → `9-Meta/Excalidraw/游戏编程模式/...`）；③ task 3.9 后 grep 全 vault `FROM "<旧目录>"` 0 命中（仅 migration-notes/design 文档自身的清单含旧路径文本，非功能性引用）；④ 0 失链（unresolved 36 → 36）。task 3.10 实际工作已分散在 batch A/B/C/E/3.9 中完成，本 task 收口验证。
- [x] 3.11 老 `0-我的笔记本.md` 内容并入新 `Dashboard.md` 后用 `obsidian delete file="0-我的笔记本"`。**完成（task 3.11）**：实测 `0-我的笔记本.md` 早已不存在（前序操作或 task 1.5 已清）。**扩展处理 4 个原计划外的边缘文件**（用户拍板）：① `LLM wiki.md` (12KB) → `2-Wiki/AI与Agent/LLM个人知识库模式.md`（保留思想原文，补 frontmatter：area=knowledge / status=stable / source=external / tags=[ai,knowledge-management,reference]，登记入 `2-Wiki/AI与Agent/_index.md`）；② `🧠 第二大脑.md` (707B) — 仪表板，`obsidian delete` 移入回收站（实测被 0 文件引用，删除后 unresolved 不变）；③ `记录灵感/📥收件箱.md` (877B) → `2-Wiki/编程语言/Python/Python 散点笔记.md`（含 venv/Pycharm/requirements 三块 hint，已是成型小知识点；`#inbox` tag 不在白名单，改为 `#programming/#python`，登记入 `2-Wiki/编程语言/_index.md` 新增 Python 节）；④ `记录灵感/` 目录变空，留待 task 3.12；unresolved 36 → 36 零失链。
- [x] 3.12 验证旧的 11 个顶级目录已为空，用 `obsidian eval code="app.vault.adapter.rmdir(...)"` 或文件管理器删除空目录（此时不会再产生失链）。**完成（task 3.12）**：实测 13 个候选目录（含 design 列表外的 Drawings/Resources）最终状态——① 已不存在（前序 batches 各文件 move 时由 `obsidian` 自动删空壳）：`Assets` `Excalidraw` `Drawings` `Resources` `日常杂谈` `软件食谱` 共 6 个；② 空但保留壳：`Templates` `开发项目` `技术文档` `英语学习` `工具用法` `读书笔记` `记录灵感` 共 7 个，本 task 用 `rmdir /s /q` 删除（git 不跟踪空目录所以无 commit 痕迹）；③ 删后 vault 顶级清洁化为：`0-Inbox/ 1-Sessions/ 2-Wiki/ 3-Projects/ 4-Journal/ 5-Life/ 6-Tools/ 9-Meta/ Dashboard.md netease/ openspec/`（8 个编号目录 + 3 保留项），完全符合 design.md 的目标结构；④ unresolved 36 → 36 零失链。
- [x] 3.13 **[Phase 2 总验收]** `obsidian unresolved` 全量返回 ≤ 旧基线值；`check_links.py` 残余断链清单 ≤ 5 条且全部为已知例外；git log 全 phase 看到的应是 R（rename）而非大量 D+A。**完成（task 3.13）**：① **unresolved**：36 → **36**（与 phase 0 基线持平，零失链 ✅）；② **check_links**：289 → **291**（+2，经核对来自 task 3.11 新加入的 `LLM个人知识库模式.md` + `Python 散点笔记.md` 两个"孤儿"—— check_links.py 把无双向引用的文件也算一次，并非真断链，Obsidian 层面 unresolved 确认无新失链）；③ **git rename 占比**：各 batch commit 显示 `post-batch-A/B/C/C-low/D/E/F/G` + `post-asset-consolidation` 全部 100% 识别为 R，其中 Batch E 13R、F 3R、G 3R、3.9 412R，主流操作 0 D+A；④ **顶级结构**：11 老目录 + 4 资产扁平目录 → **8 编号目录**（0-Inbox/1-Sessions/2-Wiki/3-Projects/4-Journal/5-Life/6-Tools/9-Meta） + 3 保留项（Dashboard.md / netease / openspec），完全符合 design.md 目标；⑤ 所有修改都有对应 git tag 作为回滚锚点（pre-phase-1 / post-batch-A..G / post-batch-C-low / post-asset-consolidation）；⑥ 所有 commit 消息可在 `git log --oneline restructure/baseline-pre-restructure..HEAD` 追溯。
