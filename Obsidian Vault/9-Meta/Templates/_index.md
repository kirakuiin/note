...
Obsidian 模板。被 ``obsidian create`` 命令通过 ``template=<name>`` 引用。包含 ``session.md`` / ``wiki-page.md`` / ``project.md`` / ``daily.md`` / ``index.md`` / ``log.md``。
...
| `wiki-page` | [[9-Meta/Templates/wiki-page.md]] | 知识页面（概述→正文→参考；可选相关链接） |
| `project` | [[9-Meta/Templates/project.md]] | 项目页面（目标→进度→日志→产出物） |
| `daily` | [[9-Meta/Templates/daily.md]] | 日记（今日→反思→明日） |
