...
- `Dashboard.md`：放在 **vault 根**。给用户自己看的行动型工作台（待办、当日入口、知识地图）
- `README.md`：放在 **git 仓库根**（这是 GitHub 实际渲染的位置；如果仓库根 = vault 根则等价）。给 GitHub 访客看的仓库简介（这是什么仓库、如何浏览、如何贡献、私有区说明）
- 旧的 `🧠 第二大脑.md`（vault 根）SHALL 被删除（其功能由 Dashboard.md 替代）

`9-Meta/AGENTS.md` 是 agent 的入口（不在根目录，但被 Dashboard 和 README 引用，详见 agent-schema 规范）。

> **架构说明**：本仓库的 git 根（`note/`）与 vault 根（`note/Obsidian Vault/`）**不重合**——vault 是 git 仓库的子目录。GitHub 渲染的是 git 根的 README.md，所以 README.md 必须放 git 根才能被访客看到；vault 根不重复维护一份。如果未来仓库结构调整为 git 根 = vault 根，README.md 物理位置自然合一，本 requirement 文本无需改动。
...
- **THEN** git 仓库根的 README.md SHALL 在仓库首页展示，用一段话说明仓库定位、推荐入口、私有区说明，并指引访客如何进入 vault 子目录
